...
static void add_tex_clamp_S_T(struct RegTable* tbl, uint64_t** alu_ptr, uint8_t tex) {
    ...
    ADD_INSTR(
        ...
        ALU_INT_TO_FLT(_R127, _x, texinfo_reg, _x) SCL_210 ALU_LAST,

        ALU_INT_TO_FLT(_R127, _y, texinfo_reg, _y) SCL_210 ALU_LAST,

        /* R127.xy = 0.5f / texSize */
        ALU_RECIP_IEEE(__, _x, _R127, _x) SCL_210 ALU_LAST,

        ALU_MUL_IEEE(_R127, _x, ALU_SRC_PS, _x, ALU_SRC_0_5, _x), ALU_RECIP_IEEE(__, _y, _R127, _y) SCL_210 ALU_LAST,

        ALU_MUL_IEEE(_R127, _y, ALU_SRC_PS, _y, ALU_SRC_0_5, _x) ALU_LAST,

        /* texCoord.xy = clamp(texCoord.xy, R127.xy, texClamp.xy) */
        ALU_MAX(__, _x, texcoord_reg, _x, _R127, _x), ALU_MAX(__, _y, texcoord_reg, _y, _R127, _y) ALU_LAST,

        ALU_MIN(texcoord_reg, _x, ALU_SRC_PV, _x, texcoord_reg, _z),
        ALU_MIN(texcoord_reg, _y, ALU_SRC_PV, _y, texcoord_reg, _w) ALU_LAST, );
}
...
static void add_tex_clamp_S(struct RegTable* tbl, uint64_t** alu_ptr, uint8_t tex) {
    ...
    ADD_INSTR(
        ...
        ALU_MUL_IEEE(_R127, _x, ALU_SRC_PS, _x, ALU_SRC_0_5, _x) ALU_LAST,

        /* texCoord.xy = clamp(texCoord.xy, R127.xy, texClamp.xy) */
        ALU_MAX(__, _x, texcoord_reg, _x, _R127, _x) ALU_LAST,

        ALU_MIN(texcoord_reg, _x, ALU_SRC_PV, _x, texcoord_reg, _z) ALU_LAST, );
}
...
static void add_tex_clamp_T(struct RegTable* tbl, uint64_t** alu_ptr, uint8_t tex) {
    ...
    ADD_INSTR(
        ...
        /* R127.y = 0.5f / texSize */
        ALU_RECIP_IEEE(__, _x, _R127, _y) SCL_210 ALU_LAST,

        ALU_MUL_IEEE(_R127, _y, ALU_SRC_PS, _x, ALU_SRC_0_5, _x) ALU_LAST,

        /* texCoord.xy = clamp(texCoord.xy, R127.xy, texClamp.xy) */
        ALU_MAX(__, _y, texcoord_reg, _y, _R127, _y) ALU_LAST,
...
static void add_mov(struct RegTable* tbl, uint64_t** alu_ptr, uint8_t src, bool single) {
    ...
    /* texel = src */
    if (single) {
        ADD_INSTR(ALU_MOV(TEXEL_REG, _w, src, _w) ALU_LAST, );
    } else {
        ADD_INSTR(ALU_MOV(TEXEL_REG, _x, src, src_alpha ? _w : _x), ALU_MOV(TEXEL_REG, _y, src, src_alpha ? _w : _y),
                  ALU_MOV(TEXEL_REG, _z, src, src_alpha ? _w : _z) ALU_LAST, );
    }
}
...
static void add_mul(struct RegTable* tbl, uint64_t** alu_ptr, uint8_t src0, uint8_t src1, bool single) {
    ...
    /* texel = src0 * src1 */
    if (single) {
        ADD_INSTR(ALU_MUL(TEXEL_REG, _w, src0, _w, src1, _w) ALU_LAST, );
    } else {
        ADD_INSTR(ALU_MUL(TEXEL_REG, _x, src0, src0_alpha ? _w : _x, src1, src1_alpha ? _w : _x),
                  ALU_MUL(TEXEL_REG, _y, src0, src0_alpha ? _w : _y, src1, src1_alpha ? _w : _y),
                  ALU_MUL(TEXEL_REG, _z, src0, src0_alpha ? _w : _z, src1, src1_alpha ? _w : _z) ALU_LAST, );
    }
}

static void add_mix(struct RegTable* tbl, uint64_t** alu_ptr, uint8_t src0, uint8_t src1, uint8_t src2, uint8_t src3,
                    bool single) {
    bool src0_alpha = (src0 == SHADER_TEXEL0A) || (src0 == SHADER_TEXEL1A);
    bool src1_alpha = (src1 == SHADER_TEXEL0A) || (src1 == SHADER_TEXEL1A);
    ...
    if (single) {
        ADD_INSTR(ALU_ADD(__, _w, src0, _w, src1 _NEG, _w) ALU_LAST,

                  ALU_MULADD(TEXEL_REG, _w, ALU_SRC_PV, _w, src2, _w, src3, _w) ALU_LAST, );
    } else {
        ADD_INSTR(ALU_ADD(__, _x, src0, src0_alpha ? _w : _x, src1 _NEG, src1_alpha ? _w : _x),
                  ALU_ADD(__, _y, src0, src0_alpha ? _w : _y, src1 _NEG, src1_alpha ? _w : _y),
                  ALU_ADD(__, _z, src0, src0_alpha ? _w : _z, src1 _NEG, src1_alpha ? _w : _z) ALU_LAST,
...
static const uint64_t noise_instructions[] = {
    ...
    ALU_MUL(__, _y, FRAG_COORD_REG, _y, _C(0), _x) ALU_LAST,

    ALU_FLOOR(_R127, _x, ALU_SRC_PV, _x),
    ALU_FLOOR(_R127, _y, ALU_SRC_PV, _y) ALU_LAST,
    ...
    ALU_MULADD(_R127, _z, ALU_SRC_PV, _z, ALU_SRC_LITERAL, _x, ALU_SRC_LITERAL, _y) ALU_LAST,
    ...
    ALU_MUL(_R127, _z, ALU_SRC_PV, _z, ALU_SRC_LITERAL, _x) ALU_LAST,
    ALU_LITERAL(0x3E22F983 /* 0.1591549367f (radians -> revolutions) */),

    ALU_SIN(_R127, _x, _R127, _x) SCL_210 ALU_LAST,

    ALU_SIN(_R127, _y, _R127, _y) SCL_210 ALU_LAST,

    ALU_SIN(_R127, _z, _R127, _z) SCL_210 ALU_LAST,
    ...
    ALU_DOT4(__, _w, ALU_SRC_LITERAL, _w, ALU_SRC_0, _x) ALU_LAST,
    ...
    ALU_MULADD(_R127, _x, _R127, _x, ALU_SRC_LITERAL, _x, ALU_SRC_0_5, _x) ALU_LAST,
    ALU_LITERAL(0x3E22F983 /* 0.1591549367f (radians -> revolutions) */),

    ALU_FRACT(__, _x, _R127, _x) ALU_LAST,

    ALU_MULADD(_R127, _x, ALU_SRC_PV, _x, ALU_SRC_LITERAL, _x, ALU_SRC_LITERAL, _y) ALU_LAST,
    ALU_LITERAL2(0x40C90FDB /* 6.283185482f (tau) */, 0xC0490FDB /* -3.141592741f (-pi) */),

    ALU_SIN(_R127, _x, _R127, _x) SCL_210 ALU_LAST,

    ALU_MUL(__, _x, _R127, _x, ALU_SRC_LITERAL, _x) ALU_LAST,
    ALU_LITERAL(0x480C63A3 /* 143758.5453f */),

    ALU_FRACT(__, _x, ALU_SRC_PV, _x) ALU_LAST,

    /* (PV + 1.0) / 2.0 */
    ALU_ADD(__, _x, ALU_SRC_PV, _x, ALU_SRC_1, _x) ALU_LAST,
    ...
    ALU_MUL(FRAG_COORD_REG, _w, ALU_SRC_PV, _x, ALU_SRC_0_5, _x) ALU_LAST,
};
...
static int generatePixelShader(GX2PixelShader* psh, struct CCFeatures* cc_features) {
    ...
    for (int i = 0; i < 2; i++) {
        if (cc_features->usedTextures[i] && cc_features->used_masks[i]) {
            ...
            ADD_INSTR(ALU_ADD(__, _x, blend_reg, _x, dst_reg _NEG, _x),
                      ALU_ADD(__, _y, blend_reg, _y, dst_reg _NEG, _y),
                      ALU_ADD(__, _z, blend_reg, _z, dst_reg _NEG, _z),
                      ALU_ADD(__, _w, blend_reg, _w, dst_reg _NEG, _w) ALU_LAST,
                      ...
                      ALU_MULADD(dst_reg, _w, ALU_SRC_PV, _w, mask_reg, _w, dst_reg, _w) ALU_LAST, );
        }
    }
    ...
    for (int c = 0; c < (cc_features->opt_2cyc ? 2 : 1); c++) {
        append_formula(&reg_table, &cur_buf, cc_features->c[c], cc_features->do_single[c][0],
                       cc_features->do_multiply[c][0], cc_features->do_mix[c][0], false);
        if (cc_features->opt_alpha) {
            append_formula(&reg_table, &cur_buf, cc_features->c[c], cc_features->do_single[c][1],
                           cc_features->do_multiply[c][1], cc_features->do_mix[c][1], true);
        }
    }
    ...
    if (cc_features->opt_fog) {
        ADD_INSTR(
            ...
            ALU_ADD(__, _x, FOG_REG, _x, _R1 _NEG, _x), ALU_ADD(__, _y, FOG_REG, _y, _R1 _NEG, _y),
            ALU_ADD(__, _z, FOG_REG, _z, _R1 _NEG, _z) ALU_LAST,
            ...
            ALU_MULADD(TEXEL_REG, _z, ALU_SRC_PV, _z, FOG_REG, _w, TEXEL_REG, _z) ALU_LAST, );
    }

    if (cc_features->opt_texture_edge && cc_features->opt_alpha) {
        ADD_INSTR(
            /* if (texel.a > 0.19) texel.a = 1.0; else discard; */
            ALU_KILLGT(__, _x, ALU_SRC_LITERAL, _x, TEXEL_REG, _w), ALU_MOV(TEXEL_REG, _w, ALU_SRC_1, _x) ALU_LAST,
    ...
    if (cc_features->opt_alpha && cc_features->opt_noise) {
        ADD_INSTR(
            ...
            ALU_MUL(__, _x, FRAG_COORD_REG, _x, ALU_SRC_LITERAL, _x) ALU_LAST, ALU_LITERAL(0x40000000 /*2.0f*/),

            ALU_ADD(__, _x, ALU_SRC_PV, _x, ALU_SRC_1 _NEG, _x) ALU_LAST,

            /* texel.a *= floor(clamp(random() + texel.a, 0.0, 1.0)); */
            ALU_ADD(__, _x, ALU_SRC_PV, _x, TEXEL_REG, _w) ALU_LAST,

            ALU_MAX(__, _x, ALU_SRC_PV, _x, ALU_SRC_0, _x) ALU_LAST,

            ALU_MIN(__, _x, ALU_SRC_PV, _x, ALU_SRC_1, _x) ALU_LAST,

            ALU_FLOOR(__, _x, ALU_SRC_PV, _x) ALU_LAST,

            ALU_MUL(TEXEL_REG, _w, TEXEL_REG, _w, ALU_SRC_PV, _x) ALU_LAST, );
    }

    if (cc_features->opt_grayscale) {
        ADD_INSTR(
            /* texel.r + texel.g + texel.b */
            ALU_ADD(__, _x, TEXEL_REG, _x, TEXEL_REG, _y) ALU_LAST,

            ALU_ADD(__, _x, ALU_SRC_PV, _x, TEXEL_REG, _z) ALU_LAST,

            /* PV.x / 3 */
            ALU_MUL_IEEE(__, _x, ALU_SRC_PV, _x, ALU_SRC_LITERAL, _x) ALU_LAST,
            ...
            ALU_MULADD(_R127, _z, GRAYSCALE_REG, _z, ALU_SRC_PV, _x, _R1 _NEG, _z) ALU_LAST,
            ...
            ALU_MULADD(TEXEL_REG, _z, ALU_SRC_PV, _z, GRAYSCALE_REG, _w, TEXEL_REG, _z) ALU_LAST, );
    }
    ...
    if (cc_features->opt_alpha) {
        if (cc_features->opt_alpha_threshold) {
            ADD_INSTR(
                ...
                ALU_KILLGT(__, _x, ALU_SRC_LITERAL, _x, TEXEL_REG, _w) ALU_LAST,
        ...
        if (cc_features->opt_invisible) {
            ADD_INSTR(
                /* texel.a = 0.0; */
                ALU_MOV(TEXEL_REG, _w, ALU_SRC_0, _x) ALU_LAST, );
        }
    }
    ...
    for (int i = 0; i < 2; i++) {
        if (cc_features->usedTextures[i]) {
            if (texclamp[i]) {
                ...
                uint64_t texinfo_buf[] = { TEX_GET_TEXTURE_INFO(dst_reg, _x, _y, _m, _m, _R1, _0, _0, _0, _0, _t(loc),
                                                                _s(loc)) };
...
static GX2AttribVar attribVars[] = {
    { "aVtxPos", GX2_SHADER_VAR_TYPE_FLOAT4, 0, 0 },         { "aTexCoord0", GX2_SHADER_VAR_TYPE_FLOAT4, 0, 1 },
    { "aTexCoord1", GX2_SHADER_VAR_TYPE_FLOAT4, 0, 2 },      { "aFog", GX2_SHADER_VAR_TYPE_FLOAT4, 0, 3 },
    { "aGrayscaleColor", GX2_SHADER_VAR_TYPE_FLOAT4, 0, 4 }, { "aInput1", GX2_SHADER_VAR_TYPE_FLOAT4, 0, 5 },
    { "aInput2", GX2_SHADER_VAR_TYPE_FLOAT4, 0, 6 },         { "aInput3", GX2_SHADER_VAR_TYPE_FLOAT4, 0, 7 },
    { "aInput4", GX2_SHADER_VAR_TYPE_FLOAT4, 0, 8 },         { "aInput5", GX2_SHADER_VAR_TYPE_FLOAT4, 0, 9 },
    { "aInput6", GX2_SHADER_VAR_TYPE_FLOAT4, 0, 10 },        { "aInput7", GX2_SHADER_VAR_TYPE_FLOAT4, 0, 11 },
};
...
static int generateVertexShader(GX2VertexShader* vsh, struct CCFeatures* cc_features) {
    ...
    // aVtxPos
    ADD_INSTR(CALL_FS NO_BARRIER, EXP_DONE(POS0, _R1, _x, _y, _z, _w), );

    // params
    for (uint32_t i = 0; i < num_ps_inputs - 1; i++) {
        ADD_INSTR(EXP(PARAM(i), _R(i + 2), _x, _y, _z, _w) NO_BARRIER, );
    }

    // last param
    ADD_INSTR((EXP_DONE(PARAM(num_ps_inputs - 1), _R(num_ps_inputs + 1), _x, _y, _z, _w) NO_BARRIER) END_OF_PROGRAM, );
...
int gx2GenerateShaderGroup(struct ShaderGroup* group, struct CCFeatures* cc_features) {
    ...
    group->fetchShader.size = GX2CalcFetchShaderSizeEx(group->numAttributes, GX2_FETCH_SHADER_TESSELLATION_NONE,
